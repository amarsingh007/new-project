...
public class YahooId extends JFrame implements ActionListener{
	JLabel lyahoo,lpass,linfoholder,title;
	JTextField tyahoo,tinfoholder;
	JButton Save,All,Back;
	JPasswordField tpass;
	public YahooId(){
		setLayout(new FlowLayout());
		
		title=new JLabel("//Yahoo Information//");
		linfoholder=new JLabel("Enter You Name");
		lyahoo=new JLabel("YahooId");
		lpass=new JLabel("YahooPassword");
		
		tinfoholder=new JTextField(25);
		tyahoo=new JTextField(25);
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			try
			{   String gmail= tyahoo.getText();
			    String pass=tpass.getText();
				String name=tinfoholder.getText();
				
				
				String sql="update  infosecurity set YahooId='"+gmail+"',YahooPassword='"+pass+"' where UserName='"+name+"'";
		...
		else if(e.getSource()==All)
		{
			try
			{
				String sql="select YahooId,YahooPassword,UserName from infosecurity";
				con=DBConnection.getConnection();
				stmt=con.createStatement();
